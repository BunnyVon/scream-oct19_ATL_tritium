...
compset=F2010-SCREAM-HR-DYAMOND2-TIRITIUM
...
case_name=SCREAMv0.${compset}.${resolution}.${date_string}_hfreq
# case_name=SCREAMv0.F2010-SCREAM-HR-DYAMOND2-TIRITIUM.AT_ne1024pg2_r0125_oRRS18to6v3.202508180952_highfreq

# Set paths
code_root=/global/cfs/cdirs/e3sm/feng809/code/scream_tritium_dev
case_root=/pscratch/sd/f/feng809/TRIDENT/simulations/${case_name}
...
if [ "${do_setup}" == "true" ]; then
    ...
    ./xmlchange SSTICE_DATA_FILENAME=/global/cfs/cdirs/e3sm/feng809/TRIDENT/input/HICCUP.sst_era5.1984-03-20.nc
    ./xmlchange SSTICE_YEAR_START=1984,SSTICE_YEAR_END=1984,SSTICE_YEAR_ALIGN=1984
    ./xmlchange GLC_AVG_PERIOD=glc_coupling_period
    ./xmlchange DOUT_S=FALSE
    ./xmlchange RUN_STARTDATE=1984-03-20
    ./xmlchange START_TOD=0

    # Edit CAM namelist to set dycore options for new grid
    cat <<EOF >> user_nl_eam

! --- Tritium Fate & Transport (1984 March case) ---

! &tritium_tracers_nl
  tritium_tracers_flag = .true.
  inject_year          = 1984
  inject_month         = 3
  inject_day           = 23
  inject_tod_in_sec    = 34800  ! 04:40 EST -> 09:40 UTC
  inject_lat_deg       = 33.2850657
  inject_lon_deg       = -81.6378505
  inject_activity_ci   = 7500.0
  half_life_yr         = 12.32

! --- History output (lightweight, hourly) ---
!empty_htapes = .true.           ! only write variables we list below
!nhtfrq       = -1               ! write every 1 hour of model time (negative = hours)
!mfilt        = 24               ! 24 samples per file (daily file if nhtfrq=-1)

! include the tritium tracer + its decay tendency; add a few winds for context
!fincl1 = 'TRIT','TRITSRC','TRITDEC','U','V','OMEGA','PS','T','Q'
    ...
    !If using dtime=100s, 9 means 15 min and 36 meanly hourly.
    nhtfrq=6,36
    mfilt=1,1
    avgflag_pertape='I','A'
    fincl1='TRIT','TRITSRC','TRITDEC','U','V','OMEGA','PS','T','Q','CLDLIQ', 'CLDICE', 'RAINQM', 'CLDRIM', 'NUMLIQ','NUMICE','NUMRAI','TGCLDLWP', 'TGCLDIWP','BVRIM', 'Z3', 'RELHUM'
    fincl2='TRIT','TRITSRC','TRITDEC','U','V','OMEGA','PS','T','Q', 'CAPE','CIN','PSL','PRECL','PRECC','PRECT','SHFLX','LHFLX','U10','TS','SST','CLDLOW','CLDMED','CLDHGH', 'CLDTOT','TMCLDLIQ', 'TMCLDICE', 'TMRAINQM', 'TMCLDRIM', 'TMQ', 'TREFHT', 'QREFHT', 'FSNTOA','FLNT', 'FLNTC', 'FSNTOAC', 'FSNS', 'FSDS', 'FLNS', 'TMNUMLIQ','TMNUMICE','TMNUMRAI','TGCLDLWP','FSUTOA' ,'SWCF','LWCF','TGCLDIWP','TUQ','TVQ','FSDSC','FSNSC','FLNSC','FLDS','FLDSC', 'FSNT','FSNTC','FLUT','FLUTC'
  
    ncdata='/global/cfs/cdirs/e3sm/feng809/TRIDENT/input/HICCUP.atm_era5.1984-03-20.ATL_ne128x8pg2.L128.nc'

    !.......................................................
    ! nudging
    !.......................................................
    Nudge_Model          = .false.
    Nudge_Path           = '/global/cfs/projectdirs/m1867/souravt/ERA5_nudging/AT_ne128x8pg2_201708/'
    Nudge_File_Template  = 'ERA5_AT_ne128x8pg2_L128.%y-%m-%d-%s.nc'
    ...
    Nudge_Hwin_lo        = 1.0
    Nudge_Hwin_hi        = 0.0
    Nudge_Hwin_lat0      = 25.0
    Nudge_Hwin_latWidth  = 40.
    Nudge_Hwin_latDelta  = 0.5
    Nudge_Hwin_lon0      = 290.
    Nudge_Hwin_lonWidth  = 75.
...
EOF
    ...
    finidat = '/pscratch/sd/f/feng809/e3sm_scratch/pm-cpu/SCREAMv0.ICRUELM.AT_ne1024pg2_r0125_oRRS18to6v3.202504070834/run/SCREAMv0.ICRUELM.AT_ne1024pg2_r0125_oRRS18to6v3.202504070834.elm.r.1984-03-20-00000.nc'
EOF
